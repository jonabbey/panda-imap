...
TCPSTREAM *tcp_aopen (NETMBX *mb,char *service,char *usrbuf)
{
  ...
  if (mb->host[0] == '[' && mb->host[i = (strlen (mb->host))-1] == ']') {
    strcpy (hostname,mb->host+1);/* yes, copy without brackets */
    hostname[i-1] = '\0';
  }
				/* note that Unix requires lowercase! */
  else if (host_name = gethostbyname (lcase (strcpy (hostname,mb->host))))
    strcpy (hostname,host_name->h_name);
				/* make command pipes */
  ...
  if (!i) {			/* if child */
    if (!fork ()) {		/* make grandchild so it's inherited by init */
      ...
      if (mb->user[0])		/* now run it */
	execl (RSHPATH,RSH,hostname,"-l",mb->user,"exec",service,0);
