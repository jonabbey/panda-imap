...
long mtx_ping (MAILSTREAM *stream)
{
  ...
  if (stream && LOCAL) {	/* only if stream already open */
	...
	((ld = lockname (lock,stream->mailbox,LOCK_SH)) >= 0)) {
      ...
      struct utimbuf times;	/* whack the times if necessary */
      LOCAL->filetime = times.actime = times.modtime = time (0);
		 ...
		 STRING *message)
{
  ...
  long f = mail_parse_flags (stream ? stream : &mtxproto,flags,&j);
...

  ...
  sprintf (tmp+26,",%ld;%010lo%02o\015\012",size,uf,(unsigned) f);
				/* write header */
...

	...
	(elt->rfc822_size = strtoul (s,&s,10)) && (!(s && *s)) &&
