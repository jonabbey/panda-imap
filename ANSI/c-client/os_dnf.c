 ...
 *
 * Author:	Mike Seibel from PC-IP version by Ken Bobey/Mark Crispin
...
long tcp_getdata (TCPSTREAM *stream)
{
  ...
  while (stream->ictr < 1) {	/* if nothing in the buffer */
	...
	((stream->ictr = read (stream->tcps,stream->ibuf,BUFLEN)) < 1)) {
      close (stream->tcps);	/* nuke the socket */
      stream->tcps = -1;
      return NIL;
    }
