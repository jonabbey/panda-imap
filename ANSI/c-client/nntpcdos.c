...
long nntp_subscribe_bboard (MAILSTREAM *stream,char *mailbox)
{
  ...
  if (s = strchr (mailbox,']')) mailbox = s + 1;
				...
				/* open .newsrc file */
  if ((fd = open (NEWSRC,O_BINARY|O_RDWR|O_CREAT,S_IREAD|S_IWRITE)) < 0) {
    mm_log ("Can't update news state",ERROR);
    return NIL;
  }
  fstat (fd,&sbuf);		/* get size of data */
  read (fd,1 + (txt = (char *) fs_get (sbuf.st_size + 2)),
	(unsigned int) sbuf.st_size);
  txt[0] = '\012';		/* make searches easier */
  txt[sbuf.st_size + 1] = '\0';	/* tie off string */
  if (sbuf.st_size &&		/* make backup file */
      (fdo =open (OLDNEWSRC,O_BINARY|O_WRONLY|O_CREAT,S_IREAD|S_IWRITE)) >= 0){
    write (fdo,txt + 1,(unsigned int) sbuf.st_size);
    close (fdo);
  }
  sprintf (tmp,"\012%s:",mailbox);
  if (strstr (txt,tmp)) {	/* already subscribed? */
...
long nntp_unsubscribe_bboard (MAILSTREAM *stream,char *mailbox)
{
  ...
  sprintf (tmp,"\012%s:",mailbox);/* see if subscribed */
...
char *nntp_slurp (MAILSTREAM *stream)
{
  ...
  LOCAL->buf[bufpos++] = '\015';/* trailing newline */
  LOCAL->buf[bufpos++] = '\012';
...
{
  unsigned int fd,ofd;
  unsigned long i,j,k,pfxlen,txtlen,sfxlen = 0;
  char *s,*sfx;
  char tmp[MAILTMPLEN];
  struct stat sbuf;
  if (!LOCAL->dirty) return;	/* never do if no updates */
  *LOCAL->buf = '\012';		/* header to make for easier searches */
				/* open .newsrc file */
  if ((fd = open (NEWSRC,O_BINARY|O_RDWR|O_CREAT,S_IREAD|S_IWRITE)) < 0) {
    mm_log ("Can't update news state",ERROR);
    return;
  ...
  *LOCAL->buf = '\012';		/* slurp the silly thing in */
  read (fd,LOCAL->buf + 1,(unsigned int) sbuf.st_size);
				/* tie off file */
  LOCAL->buf[sbuf.st_size + 1] = '\0';
  if (sbuf.st_size &&		/* make backup file */
      (ofd =open (OLDNEWSRC,O_BINARY|O_WRONLY|O_CREAT,S_IREAD|S_IWRITE)) >= 0){
    write (ofd,LOCAL->buf + 1,(unsigned int) sbuf.st_size);
    close (ofd);
  }
				/* find as subscribed newsgroup */
  sprintf (tmp,"\012%s:",LOCAL->name);
...

  if (s) {			/* found existing, calculate prefix length */
    ...
    if (s = strchr (s+1,'\012')) {/* find suffix */
  ...
  *s++ = '\015';		/* trailing newline */
  *s++ = '\012';		/* trailing newline */
  txtlen = s - tmp;		/* length of the text */
  lseek (fd,pfxlen,SEEK_SET);	/* go to end of prefix */
				/* write new text */
  write (fd,tmp,(unsigned int) txtlen);
  if (sfxlen) write (fd,sfx,(unsigned int) sfxlen);
  chsize (fd,pfxlen + txtlen + sfxlen);
  close (fd);			/* close .newsrc file */
}
...
long nntp_copy (MAILSTREAM *stream,char *sequence,char *mailbox)
{
  ...
  for (i = 1; i <= stream->nmsgs; i++)
	...
	sprintf (tmp,"%d-%s-%d %02d:%02d:%02d-%s,%ld;000000000000\015\012",
		 ...
		 i = strlen (s));
	if (write (fd,s,(unsigned int) i) < 0) {
...
char *nntp_read (void **sdb)
{
  ...
  if (!*sdb) {			/* first time through? */
    if ((fd = open (NEWSRC,O_BINARY|O_RDONLY)) < 0) return NIL;
    fstat (fd,&sbuf);		/* get file size and read data */
    read (fd,s = (char *) (*sdb = fs_get (sbuf.st_size+1)),
	  (unsigned int) sbuf.st_size);
    close (fd);			/* close file */
    s[sbuf.st_size] = '\0';	/* tie off string */
    if (t = strtok (s,"\015\012")) return t;
  }
				/* subsequent times through database */
  else if (t = strtok (NIL,"\015\012")) return t;
