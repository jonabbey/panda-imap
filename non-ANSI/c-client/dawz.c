...
void dawz_find_bboards (stream,pat)
	MAILSTREAM *stream;
	char *pat;
{
...
void dawz_fetchfast (stream,sequence)
	MAILSTREAM *stream;
	char *sequence;
{
...
void dawz_fetchflags (stream,sequence)
	MAILSTREAM *stream;
	char *sequence;
{
  return;			/* no-op for local mail */
...
void dawz_string_init (s,data,size)
	STRING *s;
	void *data;
	unsigned long size;
{
...
ENVELOPE *dawz_fetchstructure (stream,msgno,body)
	MAILSTREAM *stream;
	long msgno;
	BODY **body;
{
...

  if ((body && !*b) || !*env) {	/* have the poop we need? */
    ...
    if (read (LOCAL->fd,hdr,(unsigned int) hdrsize)) {
      ...
      rfc822_parse_msg (env,body ? b : NIL,hdr,hdrsize,&bs,"localhost",tmp);
    }
    fs_give ((void **) &tmp);
    fs_give ((void **) &hdr);
  }
...
char *dawz_fetchheader (stream,msgno)
	MAILSTREAM *stream;
	long msgno;
{
  unsigned long hdrsize;
...
/* Dawz mail fetch message text (only)
	body only;
 * Accepts: MAIL stream
 *	    message # to fetch
...
char *dawz_fetchtext (stream,msgno)
	MAILSTREAM *stream;
	long msgno;
{
  unsigned long hdrsize;
...
char *dawz_fetchbody (stream,m,s,len)
	MAILSTREAM *stream;
	long m;
	char *s;
	unsigned long *len;
...
long dawz_read (stream,count,buffer)
	MAILSTREAM *stream;
	unsigned long count;
	char *buffer;
{
...
unsigned long dawz_header (stream,msgno,size)
	MAILSTREAM *stream;
	long msgno;
	unsigned long *size;
{
...
void dawz_setflag (stream,sequence,flag)
	MAILSTREAM *stream;
	char *sequence;
	char *flag;
{
...
void dawz_clearflag (stream,sequence,flag)
	MAILSTREAM *stream;
	char *sequence;
	char *flag;
{
...
void dawz_search (stream,criteria)
	MAILSTREAM *stream;
	char *criteria;
{
  long i,n;
...
/* Dawz mail check mailbox (too)
	reparses status too;
 * Accepts: MAIL stream
 */

void dawz_check (stream)
	MAILSTREAM *stream;
{
  long i = 1;
...
long dawz_copy (stream,sequence,mailbox)
	MAILSTREAM *stream;
	char *sequence;
	char *mailbox;
{
...
long dawz_move (stream,sequence,mailbox)
	MAILSTREAM *stream;
	char *sequence;
	char *mailbox;
{
...
int dawz_copy_messages (stream,mailbox)
	MAILSTREAM *stream;
	char *mailbox;
{
  char tmp[MAILTMPLEN];
...
void dawz_update_status (stream,msgno)
	MAILSTREAM *stream;
	long msgno;
{
  char tmp[MAILTMPLEN];
...
char dawz_search_all (stream,msgno,d,n)
	MAILSTREAM *stream;
	long msgno;
	char *d;
	long n;
...
char dawz_search_answered (stream,msgno,d,n)
	MAILSTREAM *stream;
	long msgno;
	char *d;
	long n;
...
char dawz_search_deleted (stream,msgno,d,n)
	MAILSTREAM *stream;
	long msgno;
	char *d;
	long n;
...
char dawz_search_flagged (stream,msgno,d,n)
	MAILSTREAM *stream;
	long msgno;
	char *d;
	long n;
...
char dawz_search_keyword (stream,msgno,d,n)
	MAILSTREAM *stream;
	long msgno;
	char *d;
	long n;
...
char dawz_search_new (stream,msgno,d,n)
	MAILSTREAM *stream;
	long msgno;
	char *d;
	long n;
...

char dawz_search_old (stream,msgno,d,n)
	MAILSTREAM *stream;
	long msgno;
	char *d;
	long n;
...
char dawz_search_recent (stream,msgno,d,n)
	MAILSTREAM *stream;
	long msgno;
	char *d;
	long n;
...
char dawz_search_seen (stream,msgno,d,n)
	MAILSTREAM *stream;
	long msgno;
	char *d;
	long n;
...
char dawz_search_unanswered (stream,msgno,d,n)
	MAILSTREAM *stream;
	long msgno;
	char *d;
	long n;
...
char dawz_search_undeleted (stream,msgno,d,n)
	MAILSTREAM *stream;
	long msgno;
	char *d;
	long n;
...
char dawz_search_unflagged (stream,msgno,d,n)
	MAILSTREAM *stream;
	long msgno;
	char *d;
	long n;
...
char dawz_search_unkeyword (stream,msgno,d,n)
	MAILSTREAM *stream;
	long msgno;
	char *d;
	long n;
...
char dawz_search_unseen (stream,msgno,d,n)
	MAILSTREAM *stream;
	long msgno;
	char *d;
	long n;
...

char dawz_search_before (stream,msgno,d,n)
	MAILSTREAM *stream;
	long msgno;
	char *d;
	long n;
...
char dawz_search_on (stream,msgno,d,n)
	MAILSTREAM *stream;
	long msgno;
	char *d;
	long n;
...
char dawz_search_since (stream,msgno,d,n)
	MAILSTREAM *stream;
	long msgno;
	char *d;
	long n;
...
char dawz_search_body (stream,msgno,d,n)
	MAILSTREAM *stream;
	long msgno;
	char *d;
	long n;
...
char dawz_search_subject (stream,msgno,d,n)
	MAILSTREAM *stream;
	long msgno;
	char *d;
	long n;
...
char dawz_search_text (stream,msgno,d,n)
	MAILSTREAM *stream;
	long msgno;
	char *d;
	long n;
...

char dawz_search_bcc (stream,msgno,d,n)
	MAILSTREAM *stream;
	long msgno;
	char *d;
	long n;
...
char dawz_search_cc (stream,msgno,d,n)
	MAILSTREAM *stream;
	long msgno;
	char *d;
	long n;
...
char dawz_search_from (stream,msgno,d,n)
	MAILSTREAM *stream;
	long msgno;
	char *d;
	long n;
...
char dawz_search_to (stream,msgno,d,n)
	MAILSTREAM *stream;
	long msgno;
	char *d;
	long n;
...
search_t dawz_search_flag (f,n,stream)
	search_t f;
	long *n;
	MAILSTREAM *stream;
{
...
search_t dawz_search_string (f,d,n)
	search_t f;
	char **d;
	long *n;
{
