...
long bezerk_append (stream,mailbox,flags,date,message)
	MAILSTREAM *stream;
	char *mailbox;
	char *flags;
	char *date;
...
long bezerk_append_putc (fd,s,i,c)
	int fd;
	char *s;
	int *i;
	char c;
...
int bezerk_parse (stream,lock,op)
	MAILSTREAM *stream;
	char *lock;
	int op;
{
...
char bezerk_search_bcc (stream,msgno,d,n)
	...
	long n;
{
  ADDRESS *a = bezerk_fetchstructure (stream,msgno,NIL)->bcc;
...
char bezerk_search_cc (stream,msgno,d,n)
	...
	long n;
{
  ADDRESS *a = bezerk_fetchstructure (stream,msgno,NIL)->cc;
  LOCAL->buf[0] = '\0';		/* initially empty string */
				/* get text for address */
  rfc822_write_address (LOCAL->buf,a);
  return search (LOCAL->buf,(long) strlen (LOCAL->buf),d,n);
...
char bezerk_search_from (stream,msgno,d,n)
	MAILSTREAM *stream;
	long msgno;
	char *d;
	long n;
{
  ADDRESS *a = bezerk_fetchstructure (stream,msgno,NIL)->from;
  LOCAL->buf[0] = '\0';		/* initially empty string */
				/* get text for address */
  rfc822_write_address (LOCAL->buf,a);
  return search (LOCAL->buf,(long) strlen (LOCAL->buf),d,n);
...
char bezerk_search_to (stream,msgno,d,n)
	...
	long n;
{
  ADDRESS *a = bezerk_fetchstructure (stream,msgno,NIL)->to;
  LOCAL->buf[0] = '\0';		/* initially empty string */
				/* get text for address */
  rfc822_write_address (LOCAL->buf,a);
  return search (LOCAL->buf,(long) strlen (LOCAL->buf),d,n);
